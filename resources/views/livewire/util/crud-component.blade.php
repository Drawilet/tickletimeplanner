    ...
    <x-dialog-modal wire:model="modals.save">
        ...
        <x-slot name="content">
            ...
            <x-form-control class="mt-2">
                ...
                @switch($type["type"])
                @case('textarea')
                <textarea id="{{ $key }}" wire:model="data.{{ $key }}" class="textarea textarea-bordered" @foreach ($type as $key=> $value)
                                        @if ($key != 'type')
                                         {{ $key }}="{{ $value }}"
                ...
                @case('file')
                <input id="{{ $key }}" wire:model="files.{{ $key }}" type="file" class="file-input file-input-bordered" @foreach ($type as $key=> $value)
                @if ($key != 'type')
                ...
                @default
                <x-input id="{{ $key }}" wire:model="data.{{ $key }}" type="{{ $type['type'] }}" />
                @endswitch
                @endif
                ...
                <table class="table">
                    <thead class="sticky top-0 bg-base-100">
                        <tr>
                            <th></th>
                            @foreach ($keys as $key)
                            <th class="capitalize {{ $mainKey == $key ? '' : '' }}">{{ __($name."-lang.".$key) }}</th>
                            @endforeach
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="hover">
                            ...
                            <td>
                                ...
                                @else
                                <button class="btn btn-ghost" onclick="{{ $key }}{{ $item['id'] }}Modal.showModal()">
                                    @component('components.icons.arrows-pointing-out')
                                    @endcomponent
                                </button>
                                <dialog id="{{ $key }}{{ $item['id'] }}Modal" class="modal modal-bottom md:modal-middle">
                                    <div class="modal-box md:w-11/12 md:max-w-5xl">
                                        <h3 class="font-bold text-lg mb-2">{{ $Name }}
                                            {{ $key }}</h3>
                                        <div class="flex flex-wrap gap-2">
                                            @foreach ($item[$key] as $file)
                                            <img src="{{ $file['url'] }}" alt="" class="h-36  rounded border border-base-300">
                                            @endforeach
                                        </div>
