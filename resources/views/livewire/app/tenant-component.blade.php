<div class="flex flex-wrap gap-6">
    @foreach ($tenants as $tenant)
        <div class="card md:card-side shadow-xl p-4 flex-1 relative"
            style="background: linear-gradient(rgba(0, 0, 0, 0.562), rgba(255, 255, 255, 0.1)), url('{{ $tenant->background_image }}'); background-size: cover;">
            <figure class="mb-4">
                <img src="{{ $tenant->profile_image }}" alt="Foto"
                    class="w-32 h-32 object-cover rounded-full mx-auto" />
            </figure>
            <div class="card-body relative">
                <h2 class="card-title text-2xl mb-2">{{ $tenant->name }}</h2>
                <p class="mb-4">{{ $tenant->email }}</p>
                <div class="card-actions justify-end">
                    {{--     @can('app.tenants.manage')
                        <x-danger-button class="btn btn-primary" wire:click='delete({{ $tenant->id }})'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                        </x-danger-button>
                    @endcan --}}

                    <a class="btn btn-primary"
                        href="{{ route('app.tenants.show', [
                            'id' => $tenant->id,
                        ]) }}">
                        {{ __('tenant.more') }}
                    </a>
                    <a class="btn bg-red-600 hover:bg-red-800">

                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                        </svg>

                    </a>
                </div>
            </div>
        </div>
    @endforeach
</div>
